...
filenode = { 'name' : '', 'folder' : '', 'blank lines' : 0, 'comment lines' : 0, 'code lines' : 0, 'directive lines' : 0,'bytes' : 0,
...
for subdir, dirs, files in os.walk(rootdir):
	if( (not( subdir.lower().find( 'release' ) >= 0 )) 
		and (not( subdir.lower().find( 'debug' ) >= 0 )) 
