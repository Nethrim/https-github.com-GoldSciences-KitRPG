...
error_t god::setupRenderTargetImage( IGVideoContext* pManager, uint32_t nWidth, uint32_t nHeight, GODS(IGVideoImage)* out_pImage )
{
...
error_t god::setupRenderTargetViewport( uint32_t nOffsetX, uint32_t nOffsetY, uint32_t nWidth, uint32_t nHeight, god::CGViewport& out_pImage )
{
	GPtrPOD(SViewport) newRenderTargetData;
	newRenderTargetData->PositionX	= nOffsetX;//0;//
	newRenderTargetData->PositionY	= nOffsetY;//0;//
...
error_t god::setupRenderTargetSprite( GODS(IGVideoImage) pImage, GODS(IGVideoProgram) pProgram, uint32_t nOffsetX, uint32_t nOffsetY, uint32_t nWidth, uint32_t nHeight, god::CGSpriteNode& out_pSpriteRenderTarget )
{
