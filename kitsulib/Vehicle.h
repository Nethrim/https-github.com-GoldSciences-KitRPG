...
namespace klib
{
	...
	typedef CEntityVehicle CVehicleModifier;
...
//	{HP, Mana, Shield}, {HP, Mana, Shield}, {Hit, Damage, Speed, Absorption}, dropCoins, {Attack, Defend, Passive}, {Status Inflict, Status Immunity}, Name.
static const CEntityVehicle definitionsVehicle[] = 	
...
,	{{{0,0,1},	{},{0,0,0,1},0	},	{{},{}	,	{ENTITY_TECHNOLOGY_DIGITAL	, ENTITY_GRADE_MEDIUM	}},	"Jet"					}
	...
	static inline constexpr int32_t getVehicleAbsorption(const SVehicle& armor) {
		return	(int32_t)((definitionsVehicle[armor.Index].Points.Attack.Absorption + modifiersVehicle[armor.Modifier].Points.Attack.Absorption) * armor.Level);
	}

	static SEntityPoints getVehiclePoints(const SVehicle& armor) {
	...
	static SEntityFlags getVehicleFlags(const SVehicle& vehicle) {
		return definitionsVehicle[vehicle.Index].Flags | modifiersVehicle[vehicle.Modifier].Flags;
	}
