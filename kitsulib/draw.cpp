...
void blitDisplay(SGrid<_TCell, _Width, _Depth>& source, uint32_t offsetY, uint32_t offsetX, _TCell* target, size_t targetWidth, size_t targetHeight)
{
	for(uint32_t z = 0, maxZ=_Depth; z<maxZ; ++z)
		memcpy(&target[std::max((size_t)0, (offsetY+z)*targetWidth+offsetX)], &source.Cells[z][0], std::min(_Width, std::max((size_t)0, (size_t)(targetWidth-offsetX))) );
};

template<typename _TCell, size_t _Width, size_t _Depth>
void drawDisplay(SGrid<_TCell, _Width, _Depth>& source, uint32_t offsetY, uint32_t offsetX )
{
	blitDisplay(source, offsetY, offsetX, getASCIIBackBuffer(), getASCIIBackBufferWidth(), getASCIIBackBufferHeight());
};
...
void klib::drawAndPresentGame( SGame& instanceGame )
{
	...
	size_t	offset = (bbHeight-3)*bbWidth+bbWidth/2-instanceGame.UserMessage	.size()/2;	for(size_t i=0, count = instanceGame.UserMessage.size()+1; i<count; i++)	getASCIIColorBackBuffer()[offset+i] = COLOR_GREEN;
