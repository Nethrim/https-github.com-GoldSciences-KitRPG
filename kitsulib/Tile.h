...
#pragma pack(pop)
	...
	void fillCellsFromNoise( SGrid<_CellType, _Width, _Height>& grid, const _CellType& value, int64_t seed, const _CellType& clearValue = ' ', int32_t diceFaces=10 ) {
		...
		for(uint32_t i=0, count=_Width*_Height;  i<count; ++i) {
			double noise = ktools::noiseNormal(i+1, seed);
	...
	int32_t valueToGrid( SGrid<_TCell, _Width, _Height>& grid, int32_t offsetLine, int32_t offsetColumn, ktools::ALIGN align, const _TCell* values, int32_t valueCount, int32_t repeat=0 ) {
		return valueToRect( &grid.Cells[0][0], _Width, _Height, offsetLine, offsetColumn, align, values, valueCount, repeat );
	}

	template<size_t _Width, size_t _Height>
	int32_t lineToGrid( SGrid<char, _Width, _Height>& display, int32_t offsetLine, int32_t offsetColumn, ktools::ALIGN align, const char* text, uint32_t charCount = 0xFFFFFFFF ) {
	...
	int32_t printfToGrid( SGrid<_CellType, _Width, _Height>& display, int32_t offsetLine, int32_t offsetColumn, ktools::ALIGN align, const char* format, _Args... args ) {
